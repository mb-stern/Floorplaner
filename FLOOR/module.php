...
(() => {
    ...
    function automaticVariableIcon(item) {
        // Neue Darstellung hat Vorrang. Legacy bleibt wie in der sauberen Basis:
        // Das normale ObjectIcon der Variable wird verwendet, nicht Association.Icon.
        const presentationIcon = String(item?._presentationIcon || '').trim();
        if (presentationIcon) return presentationIcon;
        const objectIcon = String(item?._objectIcon || '').trim();
        if (objectIcon) return objectIcon;
    ...
    function renderSymconGlyph(icon, radius, storedSvg = '') {
        ...
        const content = svgHtml !== ''
            ? svgHtml
            : `<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="7" fill="currentColor" opacity=".55"/></svg>`;
        return `<foreignObject class="device-icon-foreign" x="${-r}" y="${-r}" width="${r * 2}" height="${r * 2}" pointer-events="none">` +
            `<div xmlns="http://www.w3.org/1999/xhtml" class="device-icon-html" style="font-size:${fontSize}px">${content}</div></foreignObject>`;
    }
    ...
    if (iconModal && iconSearch && iconList) {
        ...
        document.getElementById('iconAutoBtn')?.addEventListener('click', () => {
            ...
            item.iconManual = false;
            item.iconSvg = '';
            item.icon = automaticVariableIcon(item);
            iconModal.classList.remove('open');
            iconModal.setAttribute('aria-hidden', 'true');
            pushHistory();
            markDirty();
    ...
    function assignVariable(variableID) {
        ...
        const presentationIconFalseKey = prefix ? `_${prefix}PresentationIconFalse` : '_presentationIconFalse';
        const presentationIconTrueKey = prefix ? `_${prefix}PresentationIconTrue` : '_presentationIconTrue';
        ...
        entity[presentationIconFalseKey] = node?.presentationIconFalse || '';
        entity[presentationIconTrueKey] = node?.presentationIconTrue || '';
        if (entityType === 'item' && field === 'variableID' && entity[canActionKey] !== true) {
            entity.showDirectSlider = false;
        }
...
HTML;
                            ...
                            foreach ($meta as $key => $value) {
                                ...
                                if ($prefix === '') {
                                    ...
                                    $mapSuffix = [
                                        ...
                                        'presentationIconFalse' => 'PresentationIconFalse',
                                        'presentationIconTrue'  => 'PresentationIconTrue'
                                    ];
                                    $targetKey = '_' . $prefix . ($mapSuffix[$suffix] ?? ucfirst($suffix));
                                }
                                $Project['floors'][$floorIndex]['openings'][$openingIndex][$targetKey] = $value;
        ...
        foreach (IPS_GetChildrenIDs($ParentID) as $objectID) {
            ...
            if ($objectType === 2 && IPS_VariableExists($objectID)) {
                try {
                    ...
                    $node['presentationIconFalse'] = (string) ($meta['_presentationIconFalse'] ?? '');
                    $node['presentationIconTrue'] = (string) ($meta['_presentationIconTrue'] ?? '');
                } catch (Throwable $e) {
                    $node['valueText'] = '';
                    $this->SendDebug('ObjectTree.Variable', $e->getMessage(), 0);
                }
    ...
    private function GetVariableRuntimeMeta(int $VariableID): array
    {
        ...
        // Neue Variablendarstellungen (Symcon >= 8.1) getrennt vom Legacy-Profil auswerten.
        // Legacy bleibt bewusst unverändert: Dort verwenden wir weiterhin ObjectIcon.
        $presentationIcons = $this->GetVariablePresentationIcons($VariableID, $rawValue, $variableType);

        return [
            '_variableType'         => $variableType,
            '_objectIcon'           => (string) ($objectInfo['ObjectIcon'] ?? ''),
            '_presentationIcon'     => (string) ($presentationIcons['current'] ?? ''),
            '_presentationIconFalse'=> (string) ($presentationIcons['false'] ?? ''),
            '_presentationIconTrue' => (string) ($presentationIcons['true'] ?? ''),
            '_variablePath'   => $this->GetObjectPath($VariableID),
            '_rawValue'       => $rawValue,
            '_valueText'      => $valueText,
            '_profileName'    => $profileName,
    ...
    private function GetVariablePresentationIcons(int $VariableID, mixed $RawValue, int $VariableType): array
    {
        ...
        try {
            ...
            // Schalter-Darstellung: explizite Bool-Icons.
            ...
            if ($iconTrue !== '') {
                $result['true'] = $iconTrue;
                $result['false'] = ($useIconFalse && $iconFalse !== '') ? $iconFalse : $iconTrue;
                if ($VariableType === 0) {
                    $result['current'] = (bool) $RawValue ? $result['true'] : $result['false'];
                }
            }

            // Allgemeines/statisches Icon der Darstellung.
            $staticIcon = trim((string) ($presentation['ICON'] ?? ''));
            if ($result['current'] === '' && $staticIcon !== '') {
                $result['current'] = $staticIcon;
            }

            // Aufzählung/Wertanzeige: OPTIONS kann wertabhängige Icons enthalten.
            $optionsRaw = $presentation['OPTIONS'] ?? null;
            ...
            foreach ($options as $option) {
                ...
                $icon = trim((string) ($option['IconValue'] ?? $option['Icon'] ?? ''));
                $iconActive = array_key_exists('IconActive', $option) ? (bool) $option['IconActive'] : ($icon !== '');
                if (!$iconActive || $icon === '') {
                    continue;
                }
                ...
                $matches = false;
                if ($VariableType === 0) {
                    $matches = (bool) $optionValue === (bool) $RawValue;
                    if ((bool) $optionValue) {
                        $result['true'] = $icon;
                    } else {
                        $result['false'] = $icon;
                    }
                } elseif (is_numeric($optionValue) && is_numeric($RawValue)) {
                    $matches = (float) $optionValue === (float) $RawValue;
                } else {
            ...
            // Falls bei Bool nur die Zustandsicons gefunden wurden, aktuellen Zustand setzen.
            if ($VariableType === 0 && $result['current'] === '') {
                $candidate = (bool) $RawValue ? $result['true'] : $result['false'];
                if ($candidate !== '') {
